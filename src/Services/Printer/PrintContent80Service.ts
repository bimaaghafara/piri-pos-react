...
export class PrintContent80Service {
 ...
 orderLinesArea(lines: any) {
  ...
  orderLinesCashier(lines: any) {
    ...
    const secondLeftSpaceMaxLength = 25 + leftSpacesCount;
    const spaceTotalMaxLengt = `${lines.slice().sort((a, b) => b.total - a.total)[0].total}`.length;
    return _.map(lines, (o) => {
      lineIndex++;
      const desc = o.description;
      const descLength = desc.toString().length > 25 ? 25 : o.description.toString().length
      const firstLeftSpaces = _.range(0, leftSpacesMaxLength - o.qty.toString().length).map(() => ' ').join('');
      ...
      if (desc.toString().length > 25) {
        descSlice1 = desc.slice(0, 24) + '-';
        descSlice2 = `\x0A${lineLeftSpaces}` + desc.slice(25)
      }
      const ltotal = _.range(0, spaceTotalMaxLengt - o.total.toString().length).map(() => ' ').join('');
      return `${lineIndex > 1 ? `\x0A` : ''}` +
        `x${o.qty}${firstLeftSpaces}${desc.toString().length > 25 ? descSlice1 : o.description}${secondLeftSpaces}${ltotal}${this.formatMoney(o.total)}` +
        `${desc.toString().length > 25 ? descSlice2 : ''}` +
        `${o.description2 ? `${desc2}` : ''}`;
    }).join('\x0A');
  }
  ...
  orderSummary(order) {
    ...
    return _.map(obj, o => {
      ...
      const space = `${_.range(0, 34 - itself - maxValueSpace).map(o => { return ' ' }).join('')}`
